...
public class DijkstraTest {
    ...
    public void testShortestPathsFromStartNode() {
        // Creamos una instancia del algoritmo de Dijkstra
        DijkstraSP dijkstra = new DijkstraSP(graph, 0);

        // Validamos las distancias desde el nodo inicial (0) a otros nodos
        assertEquals(0.0, dijkstra.distTo(0), 0.001); // Distancia a sí mismo
        assertEquals(1.0, dijkstra.distTo(1), 0.001); // Nodo 0 -> Nodo 1
        assertEquals(3.0, dijkstra.distTo(2), 0.001); // Nodo 0 -> Nodo 2 (vía 1)
        assertEquals(6.0, dijkstra.distTo(3), 0.001); // Nodo 0 -> Nodo 3 (vía 2)
        assertEquals(7.0, dijkstra.distTo(4), 0.001); // Nodo 0 -> Nodo 4 (vía 3)
        assertEquals(12.0, dijkstra.distTo(5), 0.001); // Nodo 0 -> Nodo 5 (vía 4)
    }
    ...
    public void testEdgeToPaths() {
        ...
        List<DirectedEdge<Integer>> pathTo3 = dijkstra.pathTo(3); // Camino al nodo 3
